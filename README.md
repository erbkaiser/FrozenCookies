...
* **Auto-Cyclius** If enabled, turns off the Auto Pantheon. Cyclius' bonus over the course of the day can be maximized by switching him between slots periodically. This option will automate the process. For those periods where Cyclius only has negative effects, it will attempt to re-slot a god that was slotted before and fallback to Rigidel otherwise. See the schedule on https://cookieclicker.fandom.com/wiki/Pantheon
